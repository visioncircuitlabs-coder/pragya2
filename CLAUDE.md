...
- **Registration is CLOSED** — the `POST /auth/register` endpoint returns 403 and the `/register` page shows a "Coming Soon" message. This is temporary for Razorpay verification phase. Re-enable by: (1) removing the `throw ForbiddenException` in `auth.controller.ts` register method and uncommenting `this.authService.register(dto)`, (2) restoring the original register page from git history (`git show HEAD~1:client/src/app/register/page.tsx`).
- **Demo accounts** — DELETED from database on 2026-03-13. Previously: `testjobseeker@pragya.in` and `teststudent@pragya.in`.
